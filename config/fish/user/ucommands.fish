function update_bashrc -d "Convert the fish/user/ files to a single ~/.bashrc file"
	...
	set b "$HOME/.bashrc"  #- I called this var that short to longer lines
	...
	echo "## AUTOGENERATED BASHRC CONFIG:" > "$b"
	echo "##  This ~/.bashrc file was automatically generated by a Fish (yeah the" >> "$b"
	echo "##  Fish shell) function, it has some of the same aliases and variables" >> "$b"
	echo "##  that this particular Fish config has!" >> "$b"
	echo "##" >> "$b"
	echo "##  More information at: http://github.com/kevinmarquesp" >> "$b"

	#- Actually parse the Fish config to Bash script, it's a little complicated, I know...
	sed 's/^#.*//;/^ *$/d;s/  */ /g;s/^set [^ ]* \([^ ]*\) /export \1=/' "$gvars_file" >> "$b"                       #- Minify & parse the 'set -gx' syntax to 'export VARNAME='.
	awk '/\\\\$/ {printf("%s",substr($0,1,length($0)-1))} !/\\\\$/ {print}' "$aliasrc_file" |                        #- Concatenate the lines ending with \ to the next one.
		...
		sed 's/^if\(.*\)$/if\1; then/;s/end/fi/g;s/(\(.*\))/$(\1)/g;s/abbr/alias/;s/\(alias [^ =]*\) /\1=/' >> "$b"  #- Parse some of the Fish syntax to Bash.
	sed 's/^#.*//;/^ *$/d' "$bashprompt_file" >> "$b"                                                                #- Minify, just that...
